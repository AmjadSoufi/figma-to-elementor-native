...
function flag(item: FlaggedItem): void {
  ...
  const exists = _flagged.some(
    (f) => f.node_id === item.node_id && f.reason === item.reason
  );
  ...
  for (const effect of node.effects) {
    ...
    if (effect.type === 'INNER_SHADOW') {
      flag({
        ...
        nearest_native:
          'Use an outer Box Shadow with low opacity as a close approximation.',
        action: 'manual-review',
      });
    }
  }
  ...
  for (const stroke of node.strokes) {
    ...
    if (stroke.type !== 'SOLID') {
      flag({
        ...
        nearest_native:
          'Replace with a solid-color border, or export the element as a PNG image.',
        action: 'manual-review',
      });
    }
  }
...
export function checkMask(node: SceneNode): void {
  if ('isMask' in node && (node as FrameNode).isMask) {
    flag({
      ...
      category: 'visual-effect',
      reason:
        'Layer mask — Elementor does not support masking. The mask shape will be ignored.',
      nearest_native:
...
export function checkBlendMode(node: SceneNode): void {
  ...
  if (bm && bm !== 'NORMAL' && bm !== 'PASS_THROUGH') {
    flag({
      ...
      nearest_native:
        'Flatten this layer with its blend mode applied into a PNG image export.',
      action: 'export-asset',
    });
  }
}
...
export function checkMixedTextStyles(node: TextNode): void {
  if (node.fontSize === figma.mixed || node.fontName === figma.mixed || node.fills === figma.mixed) {
    flag({
      node_id: node.id,
      node_name: node.name,
      category: 'typography',
