...
export function resetWidgetCounter(): void { _widgetCounter = 0; }
...
function extractBorderSettings(node: SceneNode): {
  ...
  if (!stroke) return {};
  const weight = 'strokeWeight' in node && node.strokeWeight !== figma.mixed ? ((node as FrameNode).strokeWeight ?? 1) : 1;
...
function widget(id: string, type: WidgetType | string, settings: Record<string, unknown>): ElementorWidget {
  ...
  if ('children' in node && node.children) {
    ...
    if (textChild) {
      ...
      if (Array.isArray(textChild.fills)) {
        ...
        if (solid) textColor = rgbaToString(solid.color.r, solid.color.g, solid.color.b, solid.opacity ?? 1);
      }
    }
  }
...
export function mapIconComponent(node: SceneNode, opts: ConversionOptions): ElementorWidget {
  ...
  else if (name.includes('warning') || name.includes('alert')) iconValue = 'fas fa-exclamation-triangle';
...
export function mapIconBoxComponent(
  ...
  if ('children' in node && node.children) {
    ...
    const sorted = [...texts].sort(
      (a, b) =>
        ((b.fontSize as number) ?? 0) - ((a.fontSize as number) ?? 0)
    );
...
export function mapFlipBox(
  ...
  if ('children' in node && node.children) {
    ...
    if (sides[1] && 'children' in sides[1]) {
      const texts = (sides[1] as FrameNode).children.filter((c): c is TextNode => c.type === 'TEXT');
