...
export function fontStyleToWeight(style: string): string {
  ...
  if (s.includes("semibold") || s.includes("semi bold") || s.includes("demi") || s === "600")
    return "600";
  if (s.includes("bold") || s === "700") return "700";
  if (s.includes("extrabold") || s.includes("extra bold") || s.includes("heavy") || s === "800")
    return "800";
...
export function analyseTextNode(node: TextNode, inferHeadings: boolean): TextAnalysis {
  ...
  if (lh && typeof lh !== "symbol") {
    if (lh.unit === "PIXELS")
      lineHeight = lh.value / fontSize; // convert to em ratio
